...
`/race` is one of ClawTerminal's most distinctive features. It dispatches the same prompt to 2–4 AI models simultaneously and presents the results side-by-side, so you can compare approaches before deciding which direction to take. This tutorial covers everything from a first `/race` run to advanced use of thinking lenses.
